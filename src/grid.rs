...
impl GridMap{
    ...
    // Creates the grid of desired size at desired position(top left corner) and fills it
    // with the requested entity (with no regard to changing the transform so fuck me)

    //TODO make it so all the tiles arent fucking placed in one position
    fn new(width: usize, height: usize, position: Vec2, fill_entity: TileType) -> Self{
        let m = vec![vec![fill_entity; height]; width];
        for y in 0..height{
            for x in 0..width{
                m[x][y] = m[x][y].insert(Transform{
                    translation: Vec3::new(position.x+TILE_SIZE*x, position.y+TILE_SIZE*y, 800.0),
                    ..Default::default()
                })
            }
        }
